...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_siswa'])) {
    ...
    $pdo->prepare("INSERT INTO tbl_bk (id_siswa,id_ta,id_guru,tanggal,jenis,kategori,is_instant_sp3,deskripsi,tindakan,poin) VALUES (?,?,?,?,?,?,?,?,?,?)")
        ->execute([$id_siswa, $id_ta, $id_user, $_POST['tanggal'], $_POST['jenis'], $kategori, $is_instant_sp3, $_POST['deskripsi'], $_POST['tindakan'], $poin]);
    
    // Recalculate points for status message
...
// Query Data BK dengan detail poin tahunan (Guru tetap hanya melihat log milik sendiri, tapi poin kumulatif global)
$stmt = $pdo->prepare("SELECT b.*, s.nama, 
    ...
    WHERE b.id_guru = ? AND b.id_ta = ?
    ORDER BY b.tanggal DESC LIMIT 50");
$stmt->execute([$id_user, $ta_aktif]);
$data = $stmt->fetchAll();
...
<div class="flex flex-wrap gap-2 mb-4 no-print">
    ...
    <button onclick="window.print()" class="bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-white/10"><i class="fas fa-print mr-1"></i>Cetak Log Saya</button>
</div>

<div id="frm" class="hidden glass rounded-xl p-5 mb-6 no-print border-t-4 border-t-blue-500 shadow-2xl relative overflow-hidden">
    ...
    <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4" onsubmit="showLoading()">
        <div class="md:col-span-1">
            ...
            <select name="id_siswa" required class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none">
                <option value="">-- Pilih Siswa --</option>
                <?php foreach ($siswa as $s): ?><option value="<?= $s['id_siswa'] ?>"><?= clean($s['nama']) ?></option><?php endforeach; ?>
            </select>
        </div>
        
        <div><label class="block text-xs text-slate-400 mb-1">Tanggal</label><input type="date" name="tanggal" value="<?= date('Y-m-d') ?>" class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none"></div>
        
        <div>
            <label class="block text-xs text-slate-400 mb-1">Jenis Kasus</label>
            <select name="jenis" class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none">
        ...
        <div>
            <label class="block text-xs text-slate-400 mb-1 text-blue-400 font-bold">Kategori & Bobot</label>
            <select name="kategori" id="kat" onchange="updatePoin()" class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none">
        ...
        <div><label class="block text-xs text-slate-400 mb-1">Poin Pelanggaran</label><input type="number" name="poin" id="poin_val" value="5" class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none"></div>
        ...
        <div class="md:col-span-2"><label class="block text-xs text-slate-400 mb-1">Deskripsi / Kronologi Kejadian</label><textarea name="deskripsi" rows="2" required placeholder="Contoh: Terlambat masuk sekolah 15 menit..." class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none"></textarea></div>
        <div class="md:col-span-1"><label class="block text-xs text-slate-400 mb-1">Tindakan / Solusi yang Diberikan</label><input type="text" name="tindakan" placeholder="Tindakan yang diambil..." class="w-full bg-slate-800/80 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-blue-500 outline-none"></div>
        
        <div class="md:col-span-3 pt-2">
            <button type="submit" name="simpan" id="btnSimpan" class="bg-blue-600 hover:bg-blue-500 px-10 py-2.5 rounded-lg text-sm font-bold transition-all shadow-lg shadow-blue-600/20"><i class="fas fa-save mr-2"></i>Simpan Target & Kirim Notifikasi</button>
        </div>
    </form>
</div>
...
<div id="loadingOverlay" class="hidden fixed inset-0 z-[60] flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm no-print">
    ...
    <h3 class="text-white text-lg font-bold animate-pulse tracking-wide">Menganalisis Poin & Mengirim Notifikasi...</h3>
    <p class="text-slate-400 text-sm mt-2">Menghubungkan ke layanan PWA & WhatsApp. Mohon tunggu.</p>
</div>
...
<div class="glass rounded-xl p-5 border border-white/5 print:p-0 print:border-none print:shadow-none" id="cetak-area">
    ...
    <div class="text-center font-bold mb-6 hidden print:block text-xl text-black uppercase underline tracking-wider">LOG PENANGANAN BK (GURU: <?= clean($_SESSION['nama']) ?>)</div>
            ...
            <tr class="border-b border-white/5 hover:bg-white/5 transition-colors print:border-black">
                ...
                <td class="text-center print:p-2 print:border print:border-black">
                    <div class="flex flex-col items-center gap-1.5">
                        ...
                        <?php if($sp_level > 0): ?>
                            <a href="../admin/cetak_sp.php?id_siswa=<?= $r['id_siswa'] ?>&level=<?= $sp_level ?>" target="_blank" class="text-[10px] text-blue-400 hover:text-white hover:underline flex items-center gap-1 no-print"><i class="fas fa-file-signature"></i> Cetak SP <?= $sp_level ?></a>
                        <?php endif; ?>
                    </div>
                </td>
            </tr>
            <?php endforeach; if(empty($data)) echo '<tr><td colspan="6" class="py-12 text-center text-slate-500">Anda belum mencatat bimbingan untuk periode ini.</td></tr>'; ?>
            </tbody>
        </table>
    </div>
