<?php
$page_title = 'Konfigurasi & Daftar Nilai Siswa';
...
// 1. Simpan Konfigurasi Komponen Penilaian (Dinamis)
if (isset($_POST['simpan_komponen']) && $ta) {
    ...
    $stmt->execute([$id_guru, $id_mapel, $id_kelas, $ta['id_ta'], $json_komponen]);

    flash('msg', 'Struktur Komponen Nilai Berhasil Disimpan! Total Bobot: '.$total_bobot.'%', 'success');
...
if (isset($_POST['simpan_massal']) && $ta) {
    ...
    foreach ($_POST['id_siswa'] as $i => $id_siswa) {
        ...
        foreach ($komponen_arr as $comp) {
            ...
            $na += ($rata * ($bobot / 100)); // Tambahkan ke perhitungan NA
        }
    ...
    flash('msg', 'Seluruh Nilai Berhasil Disimpan & NA Terhitung Otomatis!', 'success');
    header('Location: raport.php?kelas='.$id_kelas.'&mapel='.$id_mapel);
    exit;
}

$stmt = $pdo->prepare("SELECT DISTINCT k.* FROM tbl_jadwal j JOIN tbl_kelas k ON j.id_kelas=k.id_kelas WHERE j.id_guru=?"); 
$stmt->execute([$id_guru]); 
$my_kelas = $stmt->fetchAll();

$stmt = $pdo->prepare("SELECT DISTINCT m.* FROM tbl_jadwal j JOIN tbl_mapel m ON j.id_mapel=m.id_mapel WHERE j.id_guru=?"); 
$stmt->execute([$id_guru]); 
$my_mapel = $stmt->fetchAll();
...
if ($sel_kelas && $sel_mapel && $ta) { 
    ...
    // Fetch Dynamic Components Settings
    $stmtB = $pdo->prepare("SELECT komponen_json FROM tbl_bobot_nilai WHERE id_guru=? AND id_mapel=? AND id_kelas=? AND id_ta=?");
...
<div class="flex flex-wrap items-center justify-between mb-6 gap-4">
    <h2 class="text-xl font-bold text-white"><i class="fas fa-file-excel mr-2 text-emerald-400"></i>Daftar Nilai Dinamis</h2>
</div>

<div class="glass rounded-xl p-5 mb-6 border-l-4 border-blue-500">
    <form method="GET" class="flex flex-wrap gap-4 items-end">
        <div>
            <label class="block text-xs text-slate-400 mb-1 font-bold uppercase tracking-widest">Pilih Kelas</label>
            <select name="kelas" onchange="this.form.submit()" class="bg-slate-800/50 border border-white/10 rounded-lg px-4 py-2 text-sm focus:border-blue-500 outline-none w-48">
                <option value="">-- Pilih --</option>
                <?php foreach ($my_kelas as $k): ?>
        ...
        <div>
            ...
            <select name="mapel" onchange="this.form.submit()" class="bg-slate-800/50 border border-white/10 rounded-lg px-4 py-2 text-sm focus:border-blue-500 outline-none w-64">
                <option value="">-- Pilih --</option>
                <?php foreach ($my_mapel as $m): ?>
...
<div class="glass rounded-xl p-5 mb-6" id="configSection" style="<?= count($komponen_arr)>0 ? 'display:none;' : '' ?>">
    <div class="flex items-center justify-between mb-4 border-b border-white/10 pb-3">
        <h3 class="font-bold text-emerald-400"><i class="fas fa-cogs mr-2"></i>Atur Komponen Penilaian</h3>
    ...
    <p class="text-xs text-slate-400 mb-4 bg-blue-500/10 p-3 rounded border border-blue-500/20">
        <i class="fas fa-info-circle mr-1"></i>Anda memegang kendali penuh! Buat kategori nilai seperti <b>"Tugas"</b> (misal 5 kotak input), <b>"Ujian Tulis"</b> (1 kotak input), dst. Sistem otomatis membuatkan header strukturnya setelah anda Simpan.
    </p>
    ...
    <form method="POST">
        ...
        <div id="komponenList" class="space-y-3 mb-4">
            <?php if (empty($komponen_arr)): ?>
                ...
                <div class="flex gap-2 items-center k-item">
                    <input type="text" name="k_nama[]" value="Tugas Harian" placeholder="Nama..." class="flex-1 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" required>
                    <input type="number" name="k_kolom[]" value="4" min="1" max="20" placeholder="Kolom" class="w-20 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" title="Jumlah Kolom Input/Pertemuan" required>
                    <div class="flex items-center w-28"><input type="number" name="k_bobot[]" value="20" min="1" max="100" class="w-full bg-slate-800 border border-white/10 rounded-l px-3 py-2 text-sm" required><span class="bg-white/10 px-2 py-2 rounded-r text-sm">%</span></div>
                ...
                <div class="flex gap-2 items-center k-item">
                    <input type="text" name="k_nama[]" value="Ulangan Harian" class="flex-1 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" required>
                    ...
                    <div class="flex items-center w-28"><input type="number" name="k_bobot[]" value="30" min="1" class="w-full bg-slate-800 border border-white/10 rounded-l px-3 py-2 text-sm" required><span class="bg-white/10 px-2 py-2 rounded-r text-sm">%</span></div>
                    <button type="button" onclick="this.parentElement.remove()" class="bg-red-500/20 text-red-400 px-3 py-2 rounded hover:bg-red-500 hover:text-white"><i class="fas fa-trash"></i></button>
                </div>
                <div class="flex gap-2 items-center k-item">
                    <input type="text" name="k_nama[]" value="PTS" class="flex-1 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" required>
                    <input type="number" name="k_kolom[]" value="1" min="1" class="w-20 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" required>
                ...
                <div class="flex gap-2 items-center k-item">
                    <input type="text" name="k_nama[]" value="PAS" class="flex-1 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" required>
                    <input type="number" name="k_kolom[]" value="1" min="1" class="w-20 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm" required>
                    <div class="flex items-center w-28"><input type="number" name="k_bobot[]" value="30" min="1" class="w-full bg-slate-800 border border-white/10 rounded-l px-3 py-2 text-sm" required><span class="bg-white/10 px-2 py-2 rounded-r text-sm">%</span></div>
                    <button type="button" onclick="this.parentElement.remove()" class="bg-red-500/20 text-red-400 px-3 py-2 rounded hover:bg-red-500 hover:text-white"><i class="fas fa-trash"></i></button>
                </div>
            <?php else: 
                ...
                <div class="flex gap-2 items-center k-item">
                    ...
                    <input type="number" name="k_kolom[]" value="<?= $c['kolom'] ?>" min="1" class="w-20 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm text-white" title="Jumlah Kolom Input" required>
        ...
        <div class="flex justify-between items-center mt-6 pt-4 border-t border-white/10">
            ...
            <button type="submit" name="simpan_komponen" class="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg"><i class="fas fa-save mr-2"></i>Simpan Konfigurasi</button>
        </div>
    </form>
</div>
...
function tambahKomponen() {
    const html = `<div class="flex gap-2 items-center k-item">
        ...
        <input type="number" name="k_kolom[]" value="1" min="1" class="w-20 bg-slate-800 border border-white/10 rounded px-3 py-2 text-sm text-white" title="Jumlah Kolom Input/Pertemuan" required>
...
<?php if (!empty($komponen_arr)): ?>
    <div class="flex flex-wrap justify-between items-center mb-4 gap-3">
        <h3 class="font-bold text-white"><i class="fas fa-list-ol mr-2 text-blue-400"></i>Form Input Nilai Siswa</h3>
        <div class="flex gap-2">
            <a href="cetak_nilai.php?kelas=<?= $sel_kelas ?>&mapel=<?= $sel_mapel ?>" target="_blank" class="bg-indigo-600 hover:bg-indigo-500 border border-white/10 px-4 py-1.5 rounded flex items-center text-xs text-white shadow-lg">
                <i class="fas fa-print mr-2"></i>Cetak Daftar Nilai
            </a>
            <button onclick="document.getElementById('configSection').style.display='block'" class="bg-slate-800 hover:bg-slate-700 border border-white/10 px-3 py-1.5 rounded text-xs text-slate-300"><i class="fas fa-cog mr-1"></i>Atur Kategori</button>
        </div>
    </div>
    ...
    <form method="POST">
        ...
        <div class="glass rounded-xl p-5 mb-20 overflow-hidden">
            <div class="overflow-x-auto pb-4 custom-scrollbar" style="width: 100%; -webkit-overflow-scrolling: touch;">
                <style>
                    ...
                    .t-input:focus { background: rgba(255,255,255,0.15); border-color: #3b82f6; outline: none; }
        ...
        <div class="fixed bottom-0 left-0 lg:ml-64 w-full lg:w-[calc(100%-16rem)] p-4 bg-slate-900 border-t border-white/10 z-40 flex justify-center backdrop-blur-sm bg-opacity-90 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
            <button type="submit" name="simpan_massal" class="bg-blue-600 hover:bg-blue-500 text-white px-10 py-3 rounded-full font-bold shadow-lg shadow-blue-500/30 uppercase tracking-widest transition-all transform hover:scale-105">
                <i class="fas fa-save mr-2"></i>Simpan Seluruh Nilai (JSON Render)
            </button>
        </div>
    </form>
<?php else: ?>
    ...
    <div class="glass rounded-xl p-10 flex flex-col items-center justify-center text-center">
        <div class="w-16 h-16 rounded-full bg-slate-800 flex items-center justify-center mb-4 border border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.5)]">
            <i class="fas fa-sliders-h text-2xl text-blue-400"></i>
        </div>
        <h3 class="text-white font-bold mb-2">Belum Tersedia Konfigurasi Nilai</h3>
        <p class="text-sm text-slate-400 max-w-sm mb-6">Silakan "Atur Komponen Ujian" terlebih dahulu (misal: Tugas, Tulis, Praktek) untuk merender Tabel Daftar Nilai yang sesuai dengan KBM Anda.</p>
        <button onclick="document.getElementById('configSection').style.display='block'" class="bg-blue-600 hover:bg-blue-500 px-6 py-2 rounded shadow-lg text-sm font-bold">Mulai Atur Komponen</button>
    </div>
<?php endif; ?>
