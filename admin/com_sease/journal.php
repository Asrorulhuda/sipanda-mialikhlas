...
if (isset($_POST['simpan'])) { 
    $pdo->prepare("INSERT INTO tbl_journal (id_guru,id_mapel,id_kelas,jam_ke,tanggal,materi,materi_akan_datang,jml_h,jml_i,jml_s,jml_a,keterangan) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")->execute([
        $_POST['id_guru'] ?? $_SESSION['user_id'], $_POST['id_mapel'], $_POST['id_kelas'], $_POST['jam_ke'], $_POST['tanggal'], 
...
if (isset($_GET['hapus'])) { $pdo->prepare("DELETE FROM tbl_journal WHERE id=?")->execute([$_GET['hapus']]); flash('msg','Dihapus!','warning'); header('Location: journal.php'); exit; }
...
$data = $pdo->query("SELECT j.*,m.nama_mapel,k.nama_kelas,g.nama as nama_guru FROM tbl_journal j LEFT JOIN tbl_mapel m ON j.id_mapel=m.id_mapel LEFT JOIN tbl_kelas k ON j.id_kelas=k.id_kelas LEFT JOIN tbl_guru g ON j.id_guru=g.id_guru ORDER BY j.tanggal DESC LIMIT 50")->fetchAll();
...
<div class="flex flex-wrap items-center justify-between mb-6 gap-4">
    <h2 class="text-xl font-bold text-white"><i class="fas fa-book-open mr-2 text-blue-400"></i>Kelola Jurnal KBM (Admin)</h2>
</div>

<div class="glass rounded-xl p-5 mb-6"><form method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-5">
    <?php if ($_SESSION['role']=='admin'): ?>
    <div class="col-span-12 md:col-span-3"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Guru</label><select name="id_guru" required class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"><option value="">-- Pilih Guru --</option><?php foreach ($guru as $g): ?><option value="<?= $g['id_guru'] ?>"><?= clean($g['nama']) ?></option><?php endforeach; ?></select></div>
    <?php endif; ?>
    <div class="col-span-12 md:col-span-2"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Tanggal</label><input type="date" name="tanggal" value="<?= date('Y-m-d') ?>" required class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"></div>
    <div class="col-span-12 md:col-span-2"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Jam Ke</label><input type="text" name="jam_ke" placeholder="1-2" required class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"></div>
    <div class="col-span-12 md:col-span-3"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Mapel</label><select name="id_mapel" required class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"><option value="">-- Pilih Mapel --</option><?php foreach ($mapel as $m): ?><option value="<?= $m['id_mapel'] ?>"><?= clean($m['nama_mapel']) ?></option><?php endforeach; ?></select></div>
    <div class="col-span-12 md:col-span-2"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Kelas</label><select name="id_kelas" required class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"><option value="">-- Pilih Kelas --</option><?php foreach ($kelas as $k): ?><option value="<?= $k['id_kelas'] ?>"><?= clean($k['nama_kelas']) ?></option><?php endforeach; ?></select></div>
    
    <div class="col-span-12 md:col-span-6"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Materi Disampaikan</label><textarea name="materi" rows="3" required class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" placeholder="Isi materi..."></textarea></div>
    <div class="col-span-12 md:col-span-6"><label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">Materi Selanjutnya</label><textarea name="materi_akan_datang" rows="3" class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" placeholder="(Opsional)..."></textarea></div>
    ...
    <div class="col-span-12 md:col-span-5 flex flex-col justify-end">
        ...
        <input type="text" name="ket" placeholder="Keterangan (Opsional)" class="w-full bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:border-blue-500 outline-none mb-3">
        <button type="submit" name="simpan" class="w-full bg-blue-600 hover:bg-blue-500 px-6 py-2.5 rounded-lg text-sm font-bold shadow-lg transition-all uppercase"><i class="fas fa-save mr-2"></i>Simpan KBM</button>
    </div>
</form></div>
...
<div class="glass rounded-xl p-5"><div class="table-container"><table class="w-full text-sm">
    ...
    <tr class="hover:bg-white/5 transition-all group">
        <td class="py-3 px-2">
            ...
            <p class="text-[10px] text-slate-400 uppercase font-bold tracking-wider mt-1"><?= clean($r['nama_guru']) ?></p>
            <p class="text-[10px] text-purple-300 font-medium"><?= clean($r['nama_kelas']) ?> <span class="mx-1 opacity-30">|</span> Jam <?= clean($r['jam_ke'] ?? '-') ?></p>
            <p class="text-[11px] text-blue-300 font-medium"><?= clean($r['nama_mapel']) ?></p>
        </td>
