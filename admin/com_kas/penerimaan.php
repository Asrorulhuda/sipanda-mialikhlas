<?php
$page_title = 'Penerimaan Kas';
require_once __DIR__ . '/../../config/init.php';
cek_role(['admin','bendahara']);
cek_fitur('kas');

if (isset($_POST['simpan'])) { $pdo->prepare("INSERT INTO tbl_penerimaan_kas (tanggal,id_jenis,uraian,jumlah,id_ta) VALUES (?,?,?,?,?)")->execute([$_POST['tanggal'],$_POST['id_jenis'],$_POST['uraian'],$_POST['jumlah'],get_ta_aktif($pdo)['id_ta']??null]); flash('msg','Berhasil!'); header('Location: penerimaan.php'); exit; }
if (isset($_GET['hapus'])) { $pdo->prepare("DELETE FROM tbl_penerimaan_kas WHERE id=?")->execute([$_GET['hapus']]); flash('msg','Dihapus!','warning'); header('Location: penerimaan.php'); exit; }

$jenis = $pdo->query("SELECT * FROM tbl_jenis_masuk ORDER BY nama")->fetchAll();
$bln = (int)($_GET['bln'] ?? date('m')); $thn = (int)($_GET['thn'] ?? date('Y'));
$stmt = $pdo->prepare("SELECT p.*,j.nama FROM tbl_penerimaan_kas p LEFT JOIN tbl_jenis_masuk j ON p.id_jenis=j.id WHERE MONTH(p.tanggal)=? AND YEAR(p.tanggal)=? ORDER BY p.tanggal DESC"); $stmt->execute([$bln, $thn]); $data = $stmt->fetchAll();
...
<div class="glass rounded-xl p-5 mb-6"><form method="POST" class="flex flex-wrap gap-3 items-end">
    ...
    <button type="submit" name="simpan" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg text-sm font-medium"><i class="fas fa-plus mr-1"></i>Tambah</button>
</form></div>

<div class="flex gap-3 mb-4">
    <form method="GET" class="flex gap-2 items-center">
        <select name="bln" class="bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm"><?php for($i=1;$i<=12;$i++): ?><option value="<?= $i ?>" <?= $bln==$i?'selected':'' ?>><?= bulan_indo($i) ?></option><?php endfor; ?></select>
        <input type="number" name="thn" value="<?= $thn ?>" class="bg-slate-800/50 border border-white/10 rounded-lg px-3 py-2 text-sm w-24">
        <button class="bg-blue-600 hover:bg-blue-500 px-3 py-2 rounded-lg text-sm"><i class="fas fa-filter"></i></button>
    </form>
    <div class="glass px-4 py-2 rounded-lg text-sm">Total: <span class="text-emerald-400 font-bold"><?= rupiah($total) ?></span></div>
</div>
...
<div class="glass rounded-xl p-5"><div class="table-container"><table class="w-full text-sm">
    ...
    <tr class="border-b border-white/5"><td class="py-2"><?= $i+1 ?></td><td><?= tgl_indo($r['tanggal']) ?></td><td><span class="px-2 py-0.5 rounded-full text-xs bg-emerald-500/20 text-emerald-400"><?= clean($r['nama']??'-') ?></span></td><td><?= clean($r['uraian']) ?></td><td class="text-emerald-400 font-medium"><?= rupiah($r['jumlah']) ?></td>
