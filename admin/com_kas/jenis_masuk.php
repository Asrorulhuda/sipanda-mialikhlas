<?php
// Generic CRUD for Jenis Masuk/Keluar Kas
$page_title = 'Jenis Kas Masuk';
$tabel = 'tbl_jenis_masuk';
require_once __DIR__ . '/../../config/init.php';
cek_role(['admin','bendahara']);
cek_fitur('kas');

if (isset($_POST['simpan'])) { $pdo->prepare("INSERT INTO $tabel (nama) VALUES (?)")->execute([$_POST['nama']]); flash('msg','Berhasil!'); header('Location: jenis_masuk.php'); exit; }
if (isset($_POST['update'])) { $pdo->prepare("UPDATE $tabel SET nama=? WHERE id=?")->execute([$_POST['nama'],$_POST['id']]); flash('msg','Berhasil!'); header('Location: jenis_masuk.php'); exit; }
if (isset($_GET['hapus'])) { $pdo->prepare("DELETE FROM $tabel WHERE id=?")->execute([$_GET['hapus']]); flash('msg','Dihapus!','warning'); header('Location: jenis_masuk.php'); exit; }
