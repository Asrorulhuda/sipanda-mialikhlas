<?php
$page_title = 'Kesehatan & Riwayat UKS';
...
// Get History
$stmt = $pdo->prepare("SELECT * FROM tbl_uks_kunjungan WHERE id_user=? AND role_user='siswa' ORDER BY created_at DESC");
$stmt->execute([$id_siswa]);
$history = $stmt->fetchAll();

// Get Latest Physical Data
$fisik_stmt = $pdo->prepare("SELECT * FROM tbl_uks_fisik WHERE id_user=? AND role_user='siswa' ORDER BY tanggal DESC LIMIT 1");
$fisik_stmt->execute([$id_siswa]);
...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-xl font-bold text-white flex items-center gap-2 font-black italic uppercase tracking-widest"><i class="fas fa-hand-holding-medical text-rose-500"></i> Layanan Kesehatan (UKS)</h2>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2">
        <div class="p-5 border-b border-white/5 bg-white/3">
            <h3 class="font-bold text-white flex items-center gap-2 text-sm uppercase tracking-widest"><i class="fas fa-history text-slate-400"></i> Riwayat Kesehatan Saya</h3>
        </div>
        <div class="p-0">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-slate-500 border-b border-white/5 bg-black/20">
                            <th class="p-4 font-bold uppercase tracking-widest text-[10px]">Tanggal & Jam</th>
                            <th class="p-4 font-bold uppercase tracking-widest text-[10px]">Keluhan</th>
                            <th class="p-4 font-bold uppercase tracking-widest text-[10px]">Diagnosa/Tindakan</th>
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/5 transition-all">
                            ...
                            <td class="p-4">
                                <p class="text-slate-300 italic max-w-xs truncate"><?= clean($h['keluhan']) ?></p>
                            ...
                            <td class="p-4">
                                ...
                                <?php else: ?>
                                    <span class="text-slate-600 italic">Belum diperiksa</span>
                                <?php endif; ?>
                            </td>
                            <td class="p-4">
                                <?php 
    ...
    <div class="glass p-6 rounded-2xl border border-white/5">
        <h3 class="font-bold text-white text-sm mb-4 border-b border-white/5 pb-2 uppercase tracking-widest"><i class="fas fa-weight text-blue-400 mr-2"></i> Update Pengukuran</h3>
        ...
        <p class="text-[9px] text-slate-500 mt-4 leading-relaxed font-medium">Berdasarkan data terakhir, BMI (Body Mass Index) Anda dihitung secara otomatis untuk memantau status gizi.</p>
    </div>
    ...
    <div class="glass p-6 rounded-2xl border border-white/5 bg-emerald-500/5">
        <h3 class="font-bold text-white text-sm mb-4 border-b border-white/5 pb-2 uppercase tracking-widest flex items-center justify-between">
            <span><i class="fas fa-pills text-emerald-400 mr-2"></i> Stok Obat UKS</span>
        ...
        <p class="text-[9px] text-slate-500 mt-4 italic font-medium leading-tight">*Data ini ditampilkan agar Anda mengetahui ketersediaan obat sebelum melakukan request ke UKS.</p>
    </div>
</div>
...
<div id="modalSakit" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[999] hidden flex items-center justify-center p-4">
    <div class="glass w-full max-w-md rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-zoom-in">
        ...
        <form id="formLaporSakit" class="p-6 space-y-4">
            ...
            <p class="text-[10px] text-slate-500 italic leading-relaxed">Laporan Anda akan langsung masuk ke Dashboard Guru UKS untuk ditindaklanjuti secara real-time.</p>
...
document.querySelectorAll('input[name="action"]').forEach(radio => {
    radio.addEventListener('change', function() {
        ...
        if (this.value === 'minta_obat') {
            ...
            document.querySelector('#modalSakit textarea').placeholder = "Cth: Pusing, demam tinggi, atau nyeri perut...";
        }
    });
});
